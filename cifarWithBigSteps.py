...
def main():
    ...
    def train(epoch_number):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            loss.backward()
            optimizer.step()
            # This next loop will continue to step the optimizer in the same direction until a local minimum in that
            # direction is found. Then it will run the gradient descent algorithm again.
            while True:
                outputs = net(inputs_masked) if args.pre_process else net(inputs)
        ...
        file_path = './records/cifar100/cifar_' + str(
            args.cifar) + '_' + args.netName + str(args.pre_process) + 'BigSteps_train.txt'
    ...
    def test(epoch_number):
        ...
        file_path = './records/cifar100/cifar_' + str(args.cifar) + '_' + args.netName + str(
            args.pre_process) + 'BigSteps_test.txt'
    ...
    statis_path = './records/cifar100/STATS_' + args.netName + str(args.pre_process) + 'BigSteps.txt'
