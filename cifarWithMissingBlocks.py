...
parser.add_argument('--missing', default=0, type=int, help='missing se block')
...
try:
    net = models.__dict__[args.netName](missing_layer=args.missing, num_classes=args.cifar)
except:
    try:
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        inputs = models.imagegradients.mask_pytorch_image_batch(inputs)
        optimizer.zero_grad()
        outputs = net(inputs)
    ...
    file_path='records/cifar_' + str(args.cifar) + '_' +args.netName+str(args.missing)+'_train.txt'
...
def test(epoch):
    ...
    file_path = 'records/cifar_' + str(args.cifar) + '_' +args.netName+ str(args.missing)+ '_test.txt'
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        save_path = './checkpoint/ckpt_cifar_' + str(args.cifar) + '_' + args.netName + str(args.missing)+'.t7'
...
statis_path = 'records/STATS_'+args.netName+str(args.missing)+'.txt'
